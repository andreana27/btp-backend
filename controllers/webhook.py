        ...
        def messenger(bot, conn):
            ...
            for entry in request.vars['entry']:
                ...
                if entry['messaging'][0].get('message'):
                    # ---------------------------------------------------------------------------------
                    req = requests.get(
                        'https://graph.facebook.com/' + chat_id + '?fields=name,first_name,last_name,profile_pic&access_token=' +
                        conn['token'])
                    jsonstring = req.json()
                    ...
                    try:
                        db.bot_storage.insert(storage_owner=chat_id,
                                              bot_id=bot.id,
                    ...
                    # -------send id_user a fb--------------------------------------------------
                    req = requests.get(
                        'https://graph.facebook.com/' + id_user + '?fields=name,first_name,last_name,profile_pic&access_token=' + token_bot)
                    jsonstring = req.json()
                    username_fb = jsonstring['name']
                    first_name = jsonstring['first_name']
                    last_name = jsonstring['last_name']
                    ...
                    if entry['messaging'][0]['postback'].get('referral'):
                        ...
                        if entry['messaging'][0]['postback']['referral'].get('ad_id'):
                            ...
                            if entry['messaging'][0]['postback']['referral'].get('ref') and \
                                    entry['messaging'][0]['postback']['referral'].get('source'):
                                import datetime
                                reference = entry['messaging'][0]['postback']['referral']['ref']
                                source = entry['messaging'][0]['postback']['referral']['source']
